...
from colse.dvine_copula_dr_joins import MultiDivineCopulaDynamicRecursive
...
def parse_args():
    ...
    parser.add_argument(
        "--output_excel_name", type=str, default="imdb_1000.xlsx", help="Name of the output excel file"
    )
    parser.add_argument(
...
def main():
    ...
    # TODO - Enable when training the error compensation model
    ...
    ms_dequantize = MultiSplineDequantizer(dataset_type)
    ms_dequantize.fit_transform()
    
    # # this will be non if there are no non-continuous columns
    # dequantized_file_name = ms_dequantize.get_dequantized_dataset_name()
    ...
    # else:
    #     datagen_load_file_name = dataset_type.get_file_path()
    ...
    if no_of_training_queries and no_of_training_queries < current_no_of_rows:
        ...
        query_joined_tables = query_joined_tables[:no_of_training_queries]
    ...
    X = np.array([get_query(ql, qr) for ql, qr, jt in zip(new_query_l, new_query_r, query_joined_tables)])
