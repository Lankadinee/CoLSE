...
from colse.cat_transform import DeQuantize, DeqDataTypes
from colse.df_utils import load_dataframe
from loguru import logger

current_dir = Path(__file__).parent
dataset_dir = get_data_path() / "census"
IS_DEQUANTIZE = True
...
def get_queries_census(**kwargs) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    """Load queries"""
    query_json = dataset_dir / "query_census13.json"
...
def query_value_mapper(query_l, query_r):
    df = load_dataframe(dataset_dir / "census.csv")
    dequantize = DeQuantize()
    quant_dict = DeQuantize.get_dequantizable_columns(df)
    ...
    for col_id, col_name in loop:
        if quant_dict[col_name].is_dequantizable:
            ...
            if quant_dict[col_name].data_type == DeqDataTypes.CATEGORICAL:
                ...
                for q_l, q_r in zip(query_l, query_r):
                    q_l[col_id] = int(q_l[col_id].astype(np.float64) + 1) if q_l[col_id] != '-inf' else '-inf'
                    q_r[col_id] = int(q_r[col_id].astype(np.float64) + 1) if q_r[col_id] != 'inf' else 'inf'
            else:
...
def get_sample_queries(no_of_queries, queried_columns: list = None, no_inf=True):
    query_l, query_r, true_card = get_queries_census()
