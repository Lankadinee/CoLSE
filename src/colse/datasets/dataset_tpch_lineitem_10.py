...
dataset_dir = get_data_path() / "tpch_lineitem_10/"
IS_DEQUANTIZE = True
MIN_DATE = "1992-01-02"
...
def generate_dataset_tpch_lineitem_10(**kwargs):
    ...
    """ Load dataset"""
    # dataset_path = get_data_path() / "tpch_lineitem_10/original_dequantized.csv"
    dataset_path = dataset_dir / (
        "original_dequantized.parquet" if IS_DEQUANTIZE else "original.parquet"
    )
    logger.info(f"Loading tpch_lineitem_10 dataset from: {dataset_path}")
...
def get_queries_tpch_lineitem_10(**kwargs) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    """Load queries"""
    query_json = dataset_dir.joinpath("query_tpch_lineitem_10.json")
    ...
    labels = pd.read_csv(
        dataset_dir.joinpath(f"label_tpch_lineitem_10_{data_split}.csv")
    )
...
def query_value_mapper(query_l, query_r):
    df = load_dataframe(dataset_dir / "original.parquet")
...
if __name__ == "__main__":
    df = generate_dataset_tpch_lineitem_10(no_of_rows=1000)
    # query_l, query_r, true_card = get_queries_tpch_lineitem_10(data_split="test")
    # print(query_l.shape)
    # print(query_r.shape)
