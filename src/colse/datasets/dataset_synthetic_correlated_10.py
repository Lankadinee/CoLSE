...
dataset_dir = get_data_path() / "synthetic_correlated_10/"
IS_DEQUANTIZE = False


def generate_dataset_correlated_10(**kwargs):
    nrows = kwargs.get("no_of_rows", 500_000)
    selected_cols = kwargs.get("selected_cols", None)

    """ Load dataset"""
    dataset_path = get_data_path() / "synthetic_correlated_10/original.csv"
    ...
    # 2 attributes
    attr1 = df["cont_attr1"].to_numpy()[:nrows]
    attr2 = df["cont_attr2"].to_numpy()[:nrows]
    attr3 = df["cont_attr3"].to_numpy()[:nrows]
    attr4 = df["cont_attr4"].to_numpy()[:nrows]
    attr5 = df["cont_attr5"].to_numpy()[:nrows]
    attr6 = df["cont_attr6"].to_numpy()[:nrows]
    attr7 = df["cont_attr7"].to_numpy()[:nrows]
    attr8 = df["cont_attr8"].to_numpy()[:nrows]
    attr9 = df["cont_attr9"].to_numpy()[:nrows]
    attr10 = df["cont_attr11"].to_numpy()[:nrows]
...
def get_queries_correlated_10(**kwargs) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    """Load queries"""
    query_json = dataset_dir.joinpath("query_synthetic_correlated_10.json")
    ...
    labels = pd.read_csv(
        dataset_dir.joinpath(f"label_synthetic_correlated_10_{data_split}.csv")
    )
