...
dataset_dir = get_data_path() / "tpch_sf2_z1_lineitem/"
IS_DEQUANTIZE = True
MIN_DATE = "1992-01-02"
...
def generate_dataset_tpch_sf2_z1_lineitem(**kwargs):
    ...
    """ Load dataset"""
    if IS_DEQUANTIZE:
        dataset_path = get_data_path() / "tpch_sf2_z1_lineitem/original_dequantized.csv"
    else:
        dataset_path = get_data_path() / "tpch_sf2_z1_lineitem/original.csv"
    logger.info(f"Loading tpch_sf2_z1_lineitem dataset from: {dataset_path}")
...
def get_queries_tpch_sf2_z1_lineitem(
    **kwargs,
    ...
    """Load queries"""
    query_json = dataset_dir.joinpath("query_tpch_sf2_z1_lineitem.json")
    ...
    labels = pd.read_csv(
        dataset_dir.joinpath(f"label_tpch_sf2_z1_lineitem_{data_split}.csv")
    )
