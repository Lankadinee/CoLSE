...
dataset_dir = get_data_path() / "synthetic_correlated_4/"
IS_DEQUANTIZE = False

def generate_dataset_correlated_4(**kwargs):
    nrows = kwargs.get("no_of_rows", 500_000)
    selected_cols = kwargs.get("selected_cols", None)

    """ Load dataset"""
    dataset_path = get_data_path() / "synthetic_correlated_4/original.csv"
    ...
    attr4 = df["cont_attr6"].to_numpy()[:nrows]
    ...
    data = np.column_stack((attr1, attr2, attr3, attr4))

    df = pd.DataFrame(data, columns=[f"{ROW_PREFIX}{i}" for i in range(1, 5)])
    # print(df.shape)
...
def get_queries_correlated_4(**kwargs) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    """Load queries"""
    query_json = dataset_dir.joinpath("query_synthetic_correlated_4.json")
    ...
    labels = pd.read_csv(dataset_dir.joinpath(f"label_synthetic_correlated_4_{data_split}.csv"))
