...
dataset_dir = get_data_path() / "tpch_sf2_z0_lineitem/"
IS_DEQUANTIZE = True
MIN_DATE = "1992-01-02"
...
def generate_dataset_tpch_sf2_z0_lineitem(**kwargs):
    ...
    """ Load dataset"""
    if IS_DEQUANTIZE:
        dataset_path = get_data_path() / "tpch_sf2_z0_lineitem/original_dequantized.csv"
    else:
        dataset_path = get_data_path() / "tpch_sf2_z0_lineitem/original.csv"
    logger.info(f"Loading tpch_sf2_z0_lineitem dataset from: {dataset_path}")
    ...
    if IS_DEQUANTIZE:
        ...
        # df["l_commitdate"] = df["l_commitdate"].apply(lambda x: datetime.strptime(x, "%Y-%m-%d"))
        # df["l_receiptdate"] = df["l_receiptdate"].apply(lambda x: datetime.strptime(x, "%Y-%m-%d"))
        # df["l_shipdate"] = df["l_shipdate"].apply(lambda x: datetime.strptime(x, "%Y-%m-%d"))
        ...
        print(min_date)
        # df["l_commitdate"] = df["l_commitdate"].apply(lambda x: (x - min_date).days)
        # df["l_receiptdate"] = df["l_receiptdate"].apply(lambda x: (x - min_date).days)
        # df["l_shipdate"] = df["l_shipdate"].apply(lambda x: (x - min_date).days)
...
def get_queries_tpch_sf2_z0_lineitem(
    **kwargs,
    ...
    """Load queries"""
    query_json = dataset_dir.joinpath("query_tpch_sf2_z0_lineitem.json")
    ...
    labels = pd.read_csv(
        dataset_dir.joinpath(f"label_tpch_sf2_z0_lineitem_{data_split}.csv")
    )
...
if __name__ == "__main__":
    # df = generate_dataset_tpch_sf2_z1_lineitem(no_of_rows=None)
    query_l, query_r, true_card = get_queries_tpch_sf2_z1_lineitem(
        data_split="test", no_of_queries=100
    )
    # print(query_l.shape)
    # print(query_r.shape)
