...
dataset_dir = get_data_path() / "synthetic_correlated_8/"
IS_DEQUANTIZE = False

def generate_dataset_correlated_8(**kwargs):
    nrows = kwargs.get("no_of_rows", 500_000)
    selected_cols = kwargs.get("selected_cols", None)

    """ Load dataset"""
    dataset_path = get_data_path() / "synthetic_correlated_8/original.csv"
    ...
    attr8 = df["cont_attr9"].to_numpy()[:nrows]
...
def get_queries_correlated_8(**kwargs) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    """Load queries"""
    query_json = dataset_dir.joinpath("query_synthetic_correlated_8.json")
    ...
    labels = pd.read_csv(dataset_dir.joinpath(f"label_synthetic_correlated_8_{data_split}.csv"))
