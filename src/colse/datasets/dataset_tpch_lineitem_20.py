...
dataset_dir = get_data_path() / "tpch_lineitem_20/"
IS_DEQUANTIZE = True
MIN_DATE = "1992-01-02"
...
def generate_dataset_tpch_lineitem_20(**kwargs):
    ...
    logger.info(f"Loading tpch_lineitem_20 dataset from: {df_path}")
    ...
    if nrows is None:
        nrows = df.shape[0]
        columns = [f"{ROW_PREFIX}{i}" for i in range(1, 16)]
        df.columns = columns
        new_df = df
    else:
    ...
    logger.info(f"Dataframe loaded, {new_df.shape}")
    return new_df


def get_queries_tpch_lineitem_20(**kwargs) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    """Load queries"""
    query_json = dataset_dir.joinpath("query_tpch_lineitem_20.json")
    ...
    labels = pd.read_csv(
        dataset_dir.joinpath(f"label_tpch_lineitem_20_{data_split}.csv")
    )
...
if __name__ == "__main__":
    query_l, query_r, true_card = get_queries_tpch_lineitem_20(data_split="test")
    # print(query_l.shape)
    # print(query_r.shape)
