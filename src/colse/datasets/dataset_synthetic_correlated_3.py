...
dataset_dir = get_data_path() / "synthetic_correlated_3/"
IS_DEQUANTIZE = False

def generate_dataset_correlated_3(**kwargs):
    nrows = kwargs.get("no_of_rows", 500_000)
    selected_cols = kwargs.get("selected_cols", None)

    """ Load dataset"""
    dataset_path = get_data_path() / "synthetic_correlated_3/original.csv"
    ...
    attr2 = df["cont_attr3"].to_numpy()[:nrows]
    attr3 = df["cont_attr4"].to_numpy()[:nrows]
    ...
    data = np.column_stack((attr1, attr2, attr3))

    df = pd.DataFrame(data, columns=[f"{ROW_PREFIX}{i}" for i in range(1, 4)])
    # print(df.shape)
...
def get_queries_correlated_3(**kwargs) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    """Load queries"""
    query_json = dataset_dir.joinpath("query_synthetic_correlated_3.json")
    ...
    labels = pd.read_csv(dataset_dir.joinpath(f"label_synthetic_correlated_3_{data_split}.csv"))
...
if __name__ == "__main__":
    query_l, query_r, true_card = get_queries_correlated_3(data_split="test")
    # print(query_l.shape)
    # print(query_r.shape)
