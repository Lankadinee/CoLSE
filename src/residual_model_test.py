...
log_file_path = LOG_ROOT.joinpath(f"evaluate-{iso_time_str}.log")
...
def test_lw_nn(output_model_path, pretrained_model_path, seed=42):
    ...
    logger.info(f"Number of validation samples: {len(test_dataset)}")
    valid_loader = DataLoader(test_dataset, batch_size=args.bs)
    ...
    for _, data in enumerate(valid_loader):
        ...
        with torch.no_grad():
            logger.info("Input shape: {}, Input content: {}".format(inputs.shape, inputs))
            preds = model(inputs).reshape(-1, args.output_len)
            logger.info("Predictions shape: {}, Predictions content: {}".format(preds.shape, preds))
